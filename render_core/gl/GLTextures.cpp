...
namespace RGL {
	namespace io {
		...
		std::vector<GLuint> Texture::textureUnitResource = initTUnitRes();
                ...
                GLint Texture::getTextureUnitID(GLuint textrueID) const {
                  assert(textrueID < mNumOfTextures);
                  return textureUnitIdx[textrueID];
                }
                GLint Texture::getTextureUnitID() const {
                  return getTextureUnitID(0);
                }
                void Texture::set(const ImgRef &flippedImg, GLuint textIdx,
                                  bool turnOnMipmap) {
                ...
                void Texture::set(const ImgRef &flippedImg, bool turnOnMipmap) {
